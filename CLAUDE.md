...
- **Images.** Every project cover is local. The two apps
  (`ams-course-explorer`, `visual-math-lab`) use real screenshots of the app,
  committed as `images/projects/<slug>.png` and downscaled to 1600px wide; the
  Visual Math Lab one is a crop of the controls and dot grid of the
  positive-predictive-value app, taken with the analytics dialog dismissed.
  Every other cover is generated by `_scripts/make_covers.py` into
  `images/projects/`. Two kinds:
...
- **All cover art is local** (§5). Nothing hot-links a remote image any
  ...
  `images/projects/*.svg`, and the two app covers are committed screenshots.
