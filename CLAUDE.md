...
- **Images.** Real figures are preferred. Projects with no natural figure use a
  generated SVG from `_scripts/make_covers.py` — typographic covers, plus one
  actual figure (`monte-carlo.svg`, a scatter-plot estimate of pi). The script is
  seeded, so re-running it is a no-op; output is committed and is not part of the
  Quarto build. Nine projects hot-link Wikimedia Commons images — see §10 before
  adding another.
...
- **Nine project cards use remote images** (§5) — eight from Wikimedia Commons,
  one from another of Apurva's GitHub Pages sites. All are under 1 MB, but the
  Wikimedia ones are third-party and can change or disappear. **Check the
  file size before adding another** — `curl -so /dev/null -w '%{size_download}'`
  — a 13.5 MB animated GIF slipped in this way and simply failed to render in
  the card. Self-hosting them requires checking each licence and attribution.
